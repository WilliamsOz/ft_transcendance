...
describe('UserChannelService', () => {
    ...
    it ('should add a user to a channel', async () => {
        ...
        jest.spyOn(prismaClient.userChannel, 'create').mockResolvedValue(expectedUserChannel);
        const userChannel = await userChannelService.addUserToChannel(userId, channelId);
    ...
    it ('should remove a user from a channel', async () => {
        ...
        jest.spyOn(prismaClient.userChannel, 'delete').mockResolvedValue(expectedUserChannel);
        const userChannel = await userChannelService.removeUserFromChannel(userId, channelId);
        expect(userChannel).toEqual(expectedUserChannel);
    });
